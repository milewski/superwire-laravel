...
{
    private function runBatch(array $batchAgentNames, array $agentOutputs, array $dynamicValues): array
    {
        $agents = [];
        ...
        if (count($agents) === 1) {
            ...
            return [
                $agentName => $this->runAgent($agents[ $agentName ], $agentOutputs, $dynamicValues),
            ];

        }

        $batchResults = $this->forkRunner()->run(...$this->batchTasks($agents, $agentOutputs, $dynamicValues));
        $resolvedResults = [];
     ...
     */
    private function batchTasks(array $agents, array $agentOutputs, array $dynamicValues): array
    {
        $tasks = [];
     ...
     * @return array<int, callable(): mixed>
     */
    private function iterationTasks(Agent $agent, array $agentOutputs, string $iterationIdentifier, array $iterationValues, array $dynamicValues): array
    {
        $tasks = [];
        ...
        foreach ($iterationValues as $iterationValue) {
            ...
            $tasks[] = function () use ($agent, $agentOutputs, $iterationIdentifier, $iterationValue, $dynamicValues): ForkExecutionFailure|AgentExecutionResult {
                ...
                return $this->executeAgentInFork(
                    agent: $agent,
    ...
    private function resolveForEachValues(Agent $agent, array $agentOutputs, array $dynamicValues): array
    {
        ...
        $resolvedValue = $this->promptParser->resolveReference($reference, $agentOutputs, [], $this->inputValues, $this->secretValues, $dynamicValues);
    ...
    private function runAgentInFork(Agent $agent, array $agentOutputs, array $dynamicValues): AgentExecutionResult|ForkExecutionFailure
    {
        try {

            return $this->runAgent($agent, $agentOutputs, $dynamicValues);
    ...
    private function executeAgentInFork(Agent $agent, string $prompt, array $outputSchema, array $agentOutputs, array $scope = [], array $dynamicValues = []): AgentExecutionResult|ForkExecutionFailure
    {
        try {

            return $this->executeAgent($agent, $prompt, $outputSchema, $agentOutputs, $scope, $dynamicValues);
